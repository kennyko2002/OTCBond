...
try:
  x=pd.read_excel(url,skiprows=3,header=None,usecols=[0,1,2,3,4,5,13],names=['ID','Name','Maturity','MaturityYear','Duration','Coupon','Tyield'])
  y=x[x.Name.notna()]
  y['recorddate']=record
...
try:
  ...
  df.columns=['ID','Name','Maturity','CouponRating','Price','TYield']
  df['Coupon']=df['CouponRating'].map(lambda x :x.split(' ')[0])
  df['Rating']=df['CouponRating'].map(lambda x :x.split(' ')[1])

  df.to_csv('twcorpbondlist',mode='w',index=False)
except:
  print("no data"+record.strftime("%Y%m%d"))
