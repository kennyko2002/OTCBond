...
url = 'https://www.tpex.org.tw/storage/bond_zone/tradeinfo/govbond//' + \
    record.strftime("%Y") + '/' + record.strftime("%Y%m") + \
    '/BDdcs001.' + record.strftime("%Y%m%d") + '-C.xls'
try:
    x = pd.read_excel(url, skiprows=4, header=None, usecols=[0, 1, 3, 4, 8, 10, 13], names=[
                      'ID', 'Name', 'Duration', 'TimeToMaturity', 'High', 'Average', 'Volume'])
    y = x[x.Name.notna()]
    y['recorddate'] = record
...
url = 'https://www.tpex.org.tw/storage/bond_zone/tradeinfo/govbond//' + \
    record.strftime("%Y") + '/' + record.strftime("%Y%m") + \
    '/BDdys503.' + record.strftime("%Y%m%d") + '-C.xls'
try:
    x = pd.read_excel(url, skiprows=3, header=None, usecols=[0, 1, 2, 3, 4, 5, 13], names=[
                      'ID', 'Name', 'Maturity', 'TimeToMaturity', 'Duration', 'Coupon', 'Yield'])
    y = x[x.Name.notna()]
    y['recorddate'] = record
...
url = 'https://nweb.tpex.org.tw/bbg/bndcrvpx_' + \
    record.strftime("%Y%m%d") + '.pdf'
try:
    df = read_pdf(url, multiple_tables=False, pages='all',
                  pandas_options={'header': None})
    df = pd.DataFrame(df[0])
    df.columns = ['ID', 'Name', 'TimeToMaturity',
                  'CouponRating', 'Price', 'Yield']
    df['Coupon'] = df['CouponRating'].map(lambda x: x.split(' ')[0])
    df['Rating'] = df['CouponRating'].map(lambda x: x.split(' ')[1])

    df.to_csv('twcorpbondlist', mode='w', index=False, columns=[
              'ID', 'Name', 'TimeToMaturity', 'Yield', 'Rating'])
except:
    print("no data twd bond list"+record.strftime("%Y%m%d"))
# phi gov
url = 'https://www.pds.com.ph/wp-content/uploads/'+record.strftime("%Y")+'/'+record.strftime(
    "%m")+'/FI-Board-Report-as-of-'+record.strftime("%B")+'-'+record.strftime("%d")+'-'+record.strftime("%Y")+'.pdf'
try:
    df = read_pdf(url, multiple_tables=False, pages='all')
    df = pd.DataFrame(df[0])
...
# phi corp
url = 'https://www.pds.com.ph/wp-content/uploads/'+record.strftime("%Y")+'/'+record.strftime(
    "%m")+'/corp_board_summary_'+record.strftime("%Y")+'-'+record.strftime("%m")+'-'+record.strftime("%d")+'.csv'
try:
    df = pd.read_csv(url)
    df['recorddate'] = record
    df = df[['Global ID', 'Local ID', 'Domestic No.', 'Coupon Rate',
             'YTM', 'Maturity', 'D.Vol(MM)', 'Last Yield', 'recorddate']]
...
urls = ['https://www.tpex.org.tw/web/bond/publish/corporate_bond_search/memo_professional.php?l=zh-tw',
        'https://www.tpex.org.tw/web/bond/publish/corporate_bond_search/memo.php?l=zh-tw']
if os.path.exists("corplistpro"):
    os.remove("corplistpro")
for url in urls:
...
# phi issur list
url = 'https://www.pds.com.ph/wp-content/uploads/'+record.strftime("%Y")+'/'+record.strftime(
    "%m")+'/Listed-Securities-as-of-'+record.strftime("%b")+'-'+record.strftime("%d")+'-'+record.strftime("%Y")+'.pdf'
try:
    ...
    df2.rename(columns={'Unnamed: 0': 'Corpname',
                        'Unnamed: 2': 'key'}, inplace=True)
